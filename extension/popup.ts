...
const i18nPopup: Record<string, Record<string, string>> = {
  en: {
...
async function decryptEntries(siteUrl: string, password: string): Promise<DecryptResponse> {
...
function setSessionExpiry(durationMs: number): void {
...
async function loadCustomSelectors(): Promise<CustomSelectors> {
  const result = await chrome.storage.local.get(['customUsernameSelector', 'customPasswordSelector']);
  return {
    usernameSelector: (result as any).customUsernameSelector || '',
    passwordSelector: (result as any).customPasswordSelector || '',
  };
}

async function saveCustomSelectors(usernameSelector: string, passwordSelector: string): Promise<void> {
...
async function applySelectorsToTab(usernameSelector: string, passwordSelector: string): Promise<void> {
  const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
  if (tabs[0]?.id) {
...
function renderEntries(entries: DecryptedEntry[], domain: string): void {
  const contentEl = document.getElementById('content')!;
  contentEl.innerHTML = `
...
async function renderSettings(): Promise<void> {
  ...
  document.getElementById('save-selectors-btn')!.addEventListener('click', async () => {
    const uSel = (document.getElementById('sel-username') as HTMLInputElement).value.trim();
    const pSel = (document.getElementById('sel-password') as HTMLInputElement).value.trim();
    ...
    setTimeout(() => { (msg as HTMLElement).style.display = 'none'; }, 2000);
  });
...
async function fillOnPage(entry: DecryptedEntry): Promise<void> {
  const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
  if (tabs[0]?.id) {
    chrome.tabs.sendMessage(tabs[0].id, {
